import pandas as pd
from imblearn.under_sampling import ClusterCentroids, NearMiss, EditedNearestNeighbours, OneSidedSelection, \
    NeighbourhoodCleaningRule, AllKNN, RepeatedEditedNearestNeighbours
...
near_miss=NearMiss(n_neighbors=28)
renn=RepeatedEditedNearestNeighbours(n_neighbors=28)
aknn = AllKNN(n_neighbors=28)
NCR = NeighbourhoodCleaningRule(n_neighbors=200)
# 定义模型
...
for fold, (train_index, test_index) in enumerate(kf.split(X), 1):
    ...
    # 应用cluster_centroids
    X_train_resampled, y_train_resampled =NCR.fit_resample(X_train_final, y_train)
