...
for fold, (train_index, test_index) in enumerate(kf.split(X), 1):
    ...
    # 应用欠采样或者过采样
    # 这里你可以选择用 over_sampler 还是 under_sampler，根据需要选择
    X_train_resampled, y_train_resampled = under_sampler.fit_resample(X_train_final, y_train)  # 或者使用 under_sampler
...
print(f"\nRandomUnderSampling Average Accuracy: {mean_acc:.4f}")
print(f"RandomUnderSampling Average Balanced Accuracy: {mean_bal_acc:.4f}")
